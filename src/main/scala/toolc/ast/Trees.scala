...
object Trees {
  ...
  case class VarDecl(tpe: TypeTree, id: Identifier, defaultValue: Option[ExprTree] = None) extends Tree with Symbolic[VariableSymbol]
  ...
  sealed case class Formal(tpe: TypeTree, id: Identifier, isVarArg: Boolean = false, defaultValue: Option[ExprTree] = None) extends Tree with Symbolic[VariableSymbol]

  sealed trait TypeTree extends Tree with Typed
  case class IntArrayType() extends TypeTree
  case class BoolArrayType() extends TypeTree
  case class StringArrayType() extends TypeTree
  case class ObjectArrayType(id: Identifier) extends TypeTree
  ...
  case class Identifier(value: String) extends TypeTree with ExprTree with Symbolic[Symbol] {
    override def getType: Type = getSymbol.getType
  }
  case class This() extends ExprTree with Symbolic[ClassSymbol]
  case class NewIntArray(size: ExprTree) extends ExprTree
  case class NewBoolArray(size: ExprTree) extends ExprTree
  case class NewStringArray(size: ExprTree) extends ExprTree
  case class NewObjectArray(size: ExprTree, tpe: Identifier) extends ExprTree
